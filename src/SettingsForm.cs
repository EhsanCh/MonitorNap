...
public class SettingsForm : Form {
    ...
    public SettingsForm() {
        ...
        for (int i = 0; i < Config.Monitors.Count; i++) {
            ...
            string text = string.Format("Monitor {0} ({1}x{2}) - Pos: [{3},{4}]", i + 2, m.Bounds.Width, m.Bounds.Height, m.Bounds.Left, m.Bounds.Top);            clbMonitors.Items.Add(text, m.IsSelected);
        }
        ...
        else if (Config.CurrentSleepAction == SleepAction.BlackScreen) cbSleepAction.SelectedIndex = 2;
        else cbSleepAction.SelectedIndex = 0;
        ...
        for (int i = 1; i <= 12; i++) cbKey.Items.Add("F" + i);
        ...
        btnSave.Click += (s, e) => {
            ...
            int mod = 0;
            if (chkAlt.Checked) mod |= 1;
            if (chkCtrl.Checked) mod |= 2;
            if (chkShift.Checked) mod |= 4;
